#!/usr/bin/env python3
"""Written avoid-hit highest-temp NO recorder (shadow 3).

RECORD ONLY. Never submits CLOB orders. Parallel to wx-near-shadow and
wx-ens-shadow. Does not modify them. Does not read POLYMARKET_LIVE_TRADING
as an arming switch.

Rules (app/weather_high_temp_strategy.md):
1. Dead leftover NO: race if an ask remains and after-fee EV > 0.
2. Still-possible NO inside the 85% coverage band: never buy.
3. Outside the band: buy only if EV >= 0.04. Zero, one, or two bins. Sit out OK.
"""
...
UA = {"User-Agent": "wx-s3-shadow/1.0", "Accept": "application/json"}
...
DEFAULT_CFG = {
    "shadow_name": "wx-s3-shadow",
    "aum_usd": "5000",
    "size_per_bin_usd": "100",
    "leftover_cap_usd": "100",
    ...
    "coverage_alpha": "0.85",
    "model_weight": "0.5",
    "spread_inflation": "0.15",
    "tick_s": 20,
    "ensemble_data_root": "/opt/ensemble-collect/data/ensemble_daily",
...
def index_ensemble(records: list[dict]) -> dict:
    out = {}
    for row in records:
        out[(row["city"], row["model"], row["valid_date"])] = row
    return out


def ensemble_members_for(
    idx: dict,
    city: str,
    day: date,
    unit: str,
    ...
    for model in MODELS:
        rec = idx.get((city, model, day.isoformat()))
        if rec is None:
            continue
        for raw in rec.get("members") or []:
            members.append(c_to_unit(D(raw), unit))
...
def observed_max_for(
    ...
    *,
    ...
    if mode is None:
        return None, "NOT_WRH_SETTLEMENT"
    key = (icao, event_date.isoformat(), mode)
    if key in cache:
        cached = cache[key]
        return (None, "WRH_NO_OBS") if cached is None else cached
    try:
        peak = wrh_event_day_max(icao, event_date, tz_name, mode)
    except Exception:  # noqa: BLE001
        cache["_fail"] = int(cache.get("_fail") or 0) + 1
        return None, "WRH_FETCH_ERROR"
    if isinstance(peak, dict) and peak.get("_error") == "MISSING_EVENT_TZ":
        return None, "MISSING_EVENT_TZ"
    if peak is None:
        cache[key] = None
        return None, "WRH_NO_OBS"
    unit_u = unit.upper()
    if mode == "hourly_f":
        if unit_u != "F":
            return None, "WRH_UNIT_MISMATCH"
        value = D(peak["max_f"])
        label = "wrh_hourly_f"
    else:
        if unit_u != "C":
...
def planned_shares(
    ...
    *,
    ...
    if sleeve == "dead_leftover":
        size_usd = D(cfg.get("leftover_cap_usd") or cfg.get("size_per_bin_usd") or "100")
    else:
...
def record_buy(store: Store, cfg: dict, *, slug: str, bucket, row: dict, extra: dict):
    ...
    sleeve = extra.get("sleeve") or "forecast_avoid"
    dead = sleeve == "dead_leftover"
    ...
    if dead:
        ...
        size_usd = D(cfg.get("size_per_bin_usd") or "100")
        p_yes = D(row["p_blend"]) if row.get("p_blend") not in (None, "") else None
        min_ev = min_edge
        if p_yes is None:
            skip_once(
                store,
                slug,
                "MISSING_P_BLEND",
                once_key=f"{extra['date']}:{bucket.no_token_id}:MISSING_P_BLEND",
                city=extra["city"],
                date=extra["date"],
                bin=bucket.title,
...
def dead_only_rows(buckets, observed_max: Decimal) -> list[dict]:
    ...
    for bucket in buckets:
        ...
        row = {
            ...
            "p_model": "0" if dead else None,
            "p_market_devig": None,
            "p_blend": "0" if dead else None,
            "in_coverage_band": False,
        ...
        if not dead:
            row["skip_reason"] = "MISSING_ENSEMBLE"
            rows.append(row)
            continue
        if bucket.no_ask is None:
            row["skip_reason"] = DEAD_NO_ASK_GONE
            rows.append(row)
            continue
        if bucket.fee_rate is None:
            ...
            rows.append(row)
            continue
        ev = no_ev(p_yes=Decimal("0"), no_ask=bucket.no_ask, fee_rate=bucket.fee_rate)
        row["ev"] = str(ev)
        row["qualify"] = ev > Decimal("0")
        if not row["qualify"]:
...
def process_event(
    ...
    observed, official = observed_max_for(
        icao,
        unit,
        wrh_cache,
        event_date=ed,
    ...
    if ens_idx is not None:
        members, n_members = ensemble_members_for(ens_idx, city, ed, unit)
    extra_base = {
        "city": city,
        "date": ed.isoformat(),
    ...
    prev_obs = store.get(f"obs:{slug}")
    store.set(f"obs:{slug}", str(observed))
    newly_dead = set()
    if prev_obs is not None:
        prev_whole = whole_degree(D(prev_obs))
        now_whole = whole_degree(observed)
        for bucket in priced:
            if (not bucket.is_dead(prev_whole)) and bucket.is_dead(now_whole):
                newly_dead.add(bucket.title)
    extra_base["newly_dead_titles"] = newly_dead
    if members:
        try:
            receipt = qualify_no_books(
                ...
                spread_inflation=D(cfg.get("spread_inflation") or "0.15"),
        ...
        rows = dead_only_rows(priced, observed)
        extra_base["coverage_band"] = ""
    by_title = {row["title"]: row for row in rows}
    for bucket in priced:
        row = by_title[bucket.title]
        if row.get("race_vs_htt"):
            stats["dead"] += 1
        sleeve = "dead_leftover" if row.get("race_vs_htt") else "forecast_avoid"
        if sleeve == "dead_leftover":
            size_usd = D(cfg.get("leftover_cap_usd") or cfg.get("size_per_bin_usd") or "100")
        else:
            size_usd = D(cfg.get("size_per_bin_usd") or "100")
        ...
        extra.pop("newly_dead_titles", None)
        extra["sleeve"] = sleeve
        ...
        if row.get("race_vs_htt") and extra_base.get("newly_dead_titles") is not None:
            if bucket.title not in extra_base["newly_dead_titles"]:
                if skip_once(
                    store,
                    slug,
                    "PREEXISTING_DEAD",
                    once_key=f"{ed.isoformat()}:{bucket.no_token_id}:PREEXISTING_DEAD",
                    **extra,
                ):
                    stats["skip"] += 1
                continue
...
def one_loop(store: Store, runtime: Path, cfg: dict) -> dict:
    extra = {
        "shadow_name": cfg.get("shadow_name") or "wx-s3-shadow",
        "record_only": True,
        "real_order_submitted": False,
        "poly_live_trading_armed": False,
        "min_edge": str(cfg.get("min_edge") or "0.04"),
    ...
    if ens_path is not None:
        try:
            ens_idx = index_ensemble(json.loads(ens_path.read_text()))
        except Exception as exc:  # noqa: BLE001
