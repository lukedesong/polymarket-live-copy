"""Forward-only real-money follower for the approved 0x9506 public wallet."""
...
SOURCE_WALLET = "0x9506e646497107cabf2d5b941a8e6a60d0db1c4f"
PROFILE_KEY = core.LIVE_PROFILE_WALLET_9506_FULL_WALLET
PROFILE_SCOPE = "FULL_WALLET"
USER_AUTHORIZED_ALLOCATION_USD = Decimal("0")
FIXED_SHARE_SCALE = Decimal("0.1")
SCALE_BASIS = "USER_AUTHORIZED_TEN_PERCENT_OF_SOURCE_SHARES_NO_SLEEVE_BUDGET"
DEFAULT_RUNTIME_DIR = Path(__file__).resolve().parent / "wallet_9506_live_runtime"


class Wallet9506ConfigurationError(RuntimeError):
    pass
...
def build_core_env(env: Mapping[str, str]) -> dict[str, str]:
    ...
    if not lock_path.is_absolute():
        raise Wallet9506ConfigurationError("SHARED_WALLET_LOCK_PATH_NOT_ABSOLUTE")
    if not coordinator_path.is_absolute():
        raise Wallet9506ConfigurationError("SHARED_WALLET_COORDINATOR_PATH_NOT_ABSOLUTE")
    values.update(
        {
...
def _stored_launch_receipt(store: LiveStore) -> dict[str, Any] | None:
    ...
    try:
        ...
        raise Wallet9506ConfigurationError(
            "INVALID_STORED_PROFILE_LAUNCH_RECEIPT"
        ) from exc
    if not isinstance(receipt, dict):
        raise Wallet9506ConfigurationError("INVALID_STORED_PROFILE_LAUNCH_RECEIPT")
    expected = {
...
def prepare_store(*, store: LiveStore, observed_at_ms: int) -> dict[str, Any]:
    ...
    if persisted is None:
        raise Wallet9506ConfigurationError("PROFILE_LAUNCH_RECEIPT_NOT_PERSISTED")
    return persisted
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="wallet-9506-live-copy")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.preflight:
        ...
        return 0
    raise Wallet9506ConfigurationError("UNHANDLED_CLI_MODE")
